...
if ! curl -fsSL --retry 3 --connect-timeout 20 "$BASE/$INSTALLER" -o "$WORK/$INSTALLER" 2>/dev/null; then
  ...
  for candidate in install-xhttp-stream-one-vlesenc-nginx.sh install-xhttp-stream-one-vlessenc-nginx.sh; do
    if curl -fsSL --retry 3 --connect-timeout 20 "$BASE/$candidate" -o "$WORK/$INSTALLER" 2>/dev/null; then break; fi
  done
